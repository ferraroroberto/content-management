...
if "%AUTO_MODE%"=="1" (
    REM Unattended: today's date, no prompts
    ...
    REM Date Selection Logic (interactive)
...
REM In auto mode this still applies (per user preference for scheduled runs);
REM the scheduled-task setting "do not close the window" is irrelevant because
REM pause holds the process here.
