...
* **Posts — skip the first note:** the first item on `https://substack.com/@<handle>` is always the newsletter teaser (a card linking to the latest paid newsletter). Drop the first unique `c-<id>` code; start collecting from the second.
