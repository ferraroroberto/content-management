@echo off
REM Visible launcher for reporting_pipeline.py - keeps CMD window open for logging.
REM Runs the daily reporting pipeline (social APIs -> Supabase -> Notion -> Substack daily Note)
REM using the virtual environment interpreter directly.
REM
REM Usage:
REM   launch_reporting.bat            - Interactive: prompts for date, asks to confirm Notion update
REM   launch_reporting.bat auto       - Unattended: today's date, auto-confirms all prompts
REM   launch_reporting.bat --auto     - Same as 'auto' (for scheduled tasks)
REM   launch_reporting.bat /auto      - Same as 'auto'
REM   launch_reporting.bat -y         - Same as 'auto'
REM   launch_reporting.bat --yes      - Same as 'auto'
REM
REM For the planning pipeline (LinkedIn -> Instagram -> Twitter -> Threads), use
REM launch_planning.bat instead.
REM
