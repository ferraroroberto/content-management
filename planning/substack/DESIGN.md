...
```
$ python -m planning.substack.daily_pipeline --dry-run
...
- One-time setup: `python -m planning.substack.bootstrap_session` opens real Chrome against the dedicated profile, lets the user log in manually, and saves the session on close. No `playwright install chromium` step is required because we drive the user's existing Chrome binary.
