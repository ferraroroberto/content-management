...
flowchart TD
    ...
    Verify{"Post scheduled" toast<br/>or composer closed<br/>within 45s?}
...
> **Locale-aware:** every user-facing label below is defined as an EN | ES regex union in [`linkedin_labels.py`](linkedin_labels.py) — the three feed share-box affordances by VISIBLE TEXT (`PHOTO_TEXT_RE`, `VIDEO_TEXT_RE`, `START_POST_TEXT_RE` — see the share-box note below), and the dialog buttons by ACCESSIBLE NAME (`ALT_TEXT_BTN_RE`, `ADD_BTN_RE`, `EXPAND_CONTENT_TYPES_RE`, `DOCUMENT_BTN_RE`, `CHOOSE_FILE_BTN_RE`, `DONE_BTN_RE`, `CONFIRM_BTN_RE`, `FINAL_SCHEDULE_BTN_RE`, `DISCARD_BTN_RE`), plus the date / time candidate helpers. That file also owns the structural anchors that carry no copy at all — `DIALOG_SEL`, `SCHEDULE_CLOCK_SEL`, and the Schedule dialog's `data-testid` selectors. When LinkedIn ships a new wording or adds a third language, extend the union in that file — do **not** re-inline a `re.compile` at the call site. The English example shown below is the EN branch only.
>
> **The modal is a native `<dialog>`, so `[role="dialog"]` alone is not enough.** LinkedIn's 2026 composer rebuild replaced `<div role="dialog">` with a native `<dialog open>` that carries **no `role` attribute** (it has the *implicit* ARIA role, which CSS attribute selectors do not see). Every helper therefore scopes through `_dialog(page)` → `DIALOG_SEL` = `[role="dialog"], dialog[open]`, which matches both shapes. `DIALOG_SEL` is a comma-separated selector *list*: never interpolate it into a bigger selector (`f'{DIALOG_SEL} button'` parses as `[role="dialog"]` OR `dialog[open] button` and silently matches the hidden `vjs-modal-dialog` leftovers in the feed) — chain off the locator instead (issue #178).
>
> **Share-box affordances are matched by visible text, not by role.** LinkedIn's redesigned feed share box mounts `Photo` / `Video` / `Start a post` as role-less `<a tabindex=0 onclick>` wrappers (the label is a child `<p>Photo</p>`); they have no `role="button"` and no aria-label once the feed hydrates. A live DOM probe confirmed `get_by_role("button", name=…)` returns **0** matches post-hydration while `get_by_text(PHOTO_TEXT_RE)` returns exactly **1** (issue #140). All three go through `linkedin_composer.click_feed_entry(page, text_re, label)`, which `get_by_text`-matches the visible `<p>` (the click bubbles up to the `<a onclick>`) and re-resolves on each attempt to ride out the cold-start + rehydration race.

| step | selector |
|------|----------|
| Open post + file picker (ILL / POST) | `click_feed_entry(page, PHOTO_TEXT_RE, "Photo", expect_selector=MEDIA_FILE_INPUT_SEL)` → `page.get_by_text(PHOTO_TEXT_RE)` on the feed. The hydrated affordance is a role-less `<a>` with a `<p>Photo</p>` label, so it's matched by whole-string visible text (`^\s*(?:photo\|foto)\s*$`) to hit `Photo` / `Foto` without colliding with feed posts that merely mention "photo"; the click bubbles to the `<a onclick>` (issue #140). `expect_selector` re-clicks if the click lands but the button's own handler hadn't hydrated yet, so nothing opened (issue #150) |
| Upload image | `input[type="file"]` (first; appears in DOM after "Photo" click) |
| Open ALT dialog | `_dialog(page) >> get_by_role("button", name=ALT_TEXT_BTN_RE)` |
| ALT textarea | `_dialog(page) >> textarea[placeholder*="describe this image" i], textarea[placeholder*="imagen" i], textarea` (first hit wins) |
| Close ALT dialog (Add) | `_dialog(page) >> get_by_role("button", name=ADD_BTN_RE).last` — the ALT panel replaces the editor body in the same `<dialog>`, so its `Add` is the only `^add$` match while it's open |
| ALT commit survived? | `_alt_commit_crashed`: polls for the editor's `Next` (fine) vs the crash screen's `get_by_role("button", name=TRY_AGAIN_RE)` (LinkedIn crashed) — see the ALT gotcha below (issue #270) |
| Editor → composer (Next) | `_dialog(page) >> button:has-text("Next"), button:has-text("Siguiente")` |
| Caption editor | `div[role="textbox"][contenteditable="true"]` (then `page.keyboard.type(...)`) |
| Open Schedule dialog | `_dialog(page) >> SCHEDULE_CLOCK_SEL` — the clock is an `<a aria-haspopup="dialog">` whose accessible name is the live counter `Scheduled (N)`, so it's matched structurally by the `svg#clock-medium` icon it wraps |
| Set date | `fill()` `input[data-testid="date-picker-input"]` with each `schedule_date_candidates(target)` rendering (`M/D/YYYY`, `D/M/YYYY`, `DD/MM/YYYY`, ISO) until LinkedIn's own summary line confirms it. Locale decides the accepted order and LinkedIn exposes it nowhere, so correctness comes from the verification, not the guess |
| Set time | Click `input[data-testid="time-picker-input"]` → `[data-testid="time-picker-menu"]` (96 fifteen-minute slots) → click the entry matching `time_picker_candidates(hour, minute)` (EN `6:30 AM` / ES 24h `06:30` / ES 12h `6:30 a. m.`) |
| Verify date+time | `[id^="schedulePostDateTimeLabel"]` renders `Posting at Tue, Jul 28, 6:30 AM` **only when both fields parse** — the authoritative oracle. `_summary_matches` additionally requires the target day number and an EN/ES month token, which is what disambiguates a date like 8/7 that parses under both orderings. Polled, not slept |
| Schedule dialog → composer | `_dialog(page) >> get_by_role("button", name=CONFIRM_BTN_RE)` (`Confirm` post-rebuild, `Next` before it). Stays `disabled` until both fields parse |
| Final Schedule button | `_dialog(page) >> get_by_role("button", name=FINAL_SCHEDULE_BTN_RE)` — the composer's primary flips `Post` → `Schedule` once a schedule is attached. Anchored on `^schedule$` / `^programar$` so it never matches the dialog's `Schedule post` tab |
| Success signal | Either LinkedIn's own `Post scheduled` toast (`POST_SCHEDULED_TOAST_RE`) appears, or the composer dialog count drops below its pre-click baseline — whichever comes first, within 45 s (`SCHEDULE_CONFIRM_TIMEOUT_MS`). The toast is the authoritative one; the composer disappearing is circumstantial and on a slow row is not observable inside any sane budget. See the confirmation note below |

**CAROUSEL-only — Add a document flow:**

| step | selector |
|------|----------|
| Open empty composer (no Photo) | `click_feed_entry(page, START_POST_TEXT_RE, "Start a post", expect_selector=COMPOSER_EDITOR_SEL)` → `page.get_by_text(START_POST_TEXT_RE)` (EN `start a post` / `create a post` / ES `empieza una publicación` / `crea una publicación`) — also role-less post-hydration (issue #140). The guard waits for the composer's caption editor inside an open modal (issue #271) |
| Expand secondary actions | `_dialog(page) >> get_by_role("button", name=EXPAND_CONTENT_TYPES_RE)` — the `+` toggle, named `Expand content types` post-rebuild and `More` before it |
| Open Share-a-document dialog | `_dialog(page) >> get_by_label(DOCUMENT_BTN_RE)` — deliberately **not** `get_by_role("button")`: the rebuilt affordance is an `<a aria-label="Document">`, a *link* |
| Push PDF | fast path: `input[type="file"]`; fallback: `page.expect_file_chooser()` + click `CHOOSE_FILE_BTN_RE` |
| Fill document title | `_dialog(page) >> get_by_label(/document title/i)` (the input has only a generated id and a bound `<label>`), then placeholder / name / text-input fallbacks. Polls for up to 30 s — the dialog mounts before the PDF preview renders |
| Wait until Done is clickable | The Done control carries `aria-disabled="true"` until the PDF is processed **and** a title is present, so this runs *after* the title is filled — polling it first deadlocks. Logs the observed state every 15 polls |
| Close document dialog (Done) | `_dialog(page) >> DONE_CTRL_SEL` — matched by **visible text**, as `<a>` or `<button>`, `:visible`-filtered (issue #237) |
...
- **LinkedIn's photo editor crashes committing ALT text, so ALT is best-effort** (issue #270). Since 2026-09, clicking the ALT panel's `Add` throws `TypeError: x.create is not a function` inside LinkedIn's own bundle and the whole app unmounts to a full-page "Something went wrong / Try again" screen. The failure used to surface as a misleading `Could not click 'Next' in the photo editor` timeout. It is not selector drift: a live probe reproduced it with `fill()` and real keystrokes, 14 and 140 characters, mouse and focus+Enter, and via the composer's `Edit` → ALT path. `_set_alt_text` now detects the crash screen and raises `AltTextCrashError`, and `schedule_one_illustration_row` re-drives the row once from a fresh feed without ALT text. The row still schedules, and its status ends in `(ALT text dropped: LinkedIn's ALT editor crashed)` so the summary shows it. Once LinkedIn fixes the bug, ALT text sticks again with no code change.
- **`get_by_role("button", name="Schedule")` is dangerous without an anchored regex** — `^schedule$` keeps it off the Schedule dialog's own `Schedule post` tab.
- **The time picker only takes a click, never a typed value.** `fill()` and per-key typing both leave a string the field re-formats and then rejects (`6:30 AM` → `6:30AM`, summary blank, `Confirm` disabled) — even for the exact format it renders by default. Open `[data-testid="time-picker-menu"]` and click the slot.
- **Never treat a written input value as proof the field took.** Both Schedule inputs happily keep an unparsed string; the only trustworthy signal is `[id^="schedulePostDateTimeLabel"]` rendering `Posting at …`, and `Confirm` un-disabling. It also renders **asynchronously** — a fixed `wait_for_timeout(700)` after the fill passed five rows in a row and then failed on a date LinkedIn was in fact accepting. Poll it (`_wait_for_summary_match`), don't sleep on it.
- **`get_attribute("disabled")` returns `""` for `<button disabled>` — which is falsy.** So `if not disabled:` reports every disabled button as ready. This silently turned the carousel's "wait for PDF processing" into a no-op, handing a still-mounting dialog to the next step. Test presence (`is not None`) via `_button_is_enabled`.
- **The Share-a-document `Done` button gates on the title, not just the upload.** It stays disabled until a non-empty title is typed, so any "wait for Done, then fill the title" ordering deadlocks until the timeout. Fill the title first, then wait for `Done`.

- **`Done` is a role-less `<a>`, not a button** (issue #237). LinkedIn re-rendered it in 2026-08 as `<a aria-disabled="false">` wrapping nested `<span>`s — no `role`, no `href`, no `aria-label`. An `<a>` without `href` or `role` has **no implicit ARIA button role**, so `get_by_role("button", …)` cannot match it at all; it silently found nothing for 177 consecutive polls while the dialog sat plainly ready on screen, and the wait reported "PDF processing did not finish". Same pattern as the feed share-box in #140. Match it by visible text via `DONE_CTRL_SEL`. **Keep the `:visible` filter** — the only element in the document whose text is exactly `Done` is video.js's *hidden* caption-settings button (`button.vjs-done-button` inside `div.vjs-modal-dialog.vjs-hidden[role="dialog"]`), which `DIALOG_SEL` matches; `get_by_role` used to exclude it for free by skipping hidden nodes, a text anchor does not.

- **When a wait times out, make it say what it saw.** `_wait_for_pdf_upload` logs the Done control's observed state every 15 polls and folds it into the raised error. `present=0` means the selector matched nothing (drift); `present=1 aria-disabled='true'` means it matched and is genuinely still processing. That one distinction is what separated #237 from a suspected slow-LinkedIn transient — the failure screenshot alone was actively misleading, showing a dialog that looked completely ready.
...
- **Carousel "Next" buttons on the feed** behind the modal have `aria-label="Next"` but empty visible text. Scope through `_dialog(page)` and match on visible text (`button:has-text("Next"), button:has-text("Siguiente")`) so they don't win the `.first` race.
- **Don't trust a fixed `wait_for_timeout` + screenshot as success confirmation.** The composer briefly stays open while LinkedIn renders the schedule, then unmounts. Wait for an actual confirmation signal — see the next bullet for which.
- **The composer closing is not a reliable confirmation on its own, and 20 s was not enough.** The first real `--live` run reported 2 of 7 rows as failures that had actually been scheduled: their `-live-FAIL.png` screenshots are indistinguishable from the successful rows' `-live-after.png` — composer gone, feed restored, LinkedIn's `Post scheduled` toast on screen. The old wait compared the composer count against a pre-click baseline for 20 s and asserted `post likely NOT scheduled` on timeout, which it had no way to observe. Because the caller only unticks **Work in Progress LI** on success, both rows stayed queued and the next run would have scheduled them a second time. The wait now lives in `linkedin_composer.wait_for_schedule_confirmation` (one copy, shared by the photo and video drivers, because the two must agree on what "scheduled" means), runs to 45 s, and accepts LinkedIn's own toast as the authoritative signal. It also **refuses to infer success from a zero baseline** — if the composer wasn't matched before the click, "the count dropped" can never be true and "the count is zero" was already true, so only the toast can confirm that row. Reporting success without positive evidence is how a genuinely missing post would get silently unticked and dropped (issue #178).
- **The composer re-mounts late after the Schedule dialog's Confirm.** Confirm closes the Schedule dialog and LinkedIn re-renders the composer from scratch; for a video a live probe measured ~2.1 s, longer than the 1.5 s the callers sleep. A snapshot taken in that gap matched zero composers, which silently turned the row into toast-only — and a video's toast often lands after the 45 s wait, so a scheduled post was reported FAIL and left queued for a duplicate. `schedule_pre_state` now polls up to 10 s (`COMPOSER_REMOUNT_TIMEOUT_MS`) for the composer before snapshotting, logging how long the re-mount took; if it never comes back the zero-baseline rule above still applies (issue #319).
- **A leftover toast can confirm the wrong row.** All rows share one browser session, so the previous row's `Post scheduled` toast may still be on screen when the next row clicks Schedule. `schedule_pre_state` snapshots whether a toast is *already* visible immediately before the click, and the waiter ignores the toast signal for that row when it is — falling back to the composer signal rather than trusting a stale confirmation.
...
- **The Schedule dialog's calendar and time popovers render in floating-ui portals, outside the `<dialog>` subtree.** Dumping the dialog's `outerHTML` shows only an `aria-owns` stub where the popover content should be. The driver avoids them entirely — it types the date and clicks the time menu page-wide — but any future work against those popovers must query document-wide, not through `_dialog(page)`. (The pre-rebuild artdeco calendar had the same property for a different reason.)
- **The feed share box is matched by visible text, and the first action of a fresh session needs a retry budget.** LinkedIn redirects `/feed/` → `/` for logged-in users and renders the share box client-side; the cold-start race used to time the original 10 s `.click()` out (issue #27). LinkedIn then **redesigned the share box** so `Photo` / `Video` / `Start a post` mount as role-less `<a tabindex=0 onclick>` wrappers (label in a child `<p>`) — `get_by_role("button", name=…)` returns **0** matches post-hydration, and because the pre-hydration placeholder briefly *is* a `<div role="button">`, the old selector would catch it, see it swapped out, and spin on Playwright's "element was detached from the DOM, retrying" loop for the full 30 s (issue #140). The fix: every feed-entry call site (`_click_add_photo`, `_click_start_a_post`, `_click_video_button`) goes through `linkedin_composer.click_feed_entry(page, text_re, label)`, which `get_by_text`-matches the visible label (the click bubbles to the `<a onclick>`) and re-resolves on each attempt with a short per-attempt timeout (`FEED_ENTRY_ATTEMPT_TIMEOUT_MS`, 6 s), looping until the overall budget (`FEED_ENTRY_CLICK_TIMEOUT_MS`, 30 s). A miss fails fast and the next attempt clicks against the settled DOM; a warm share box still returns on the first attempt in ~1 s.
- **A feed-entry click can succeed and still do nothing.** Distinct from #27/#140: the target `<a onclick>` can pass every Playwright actionability check (attached, visible, stable, receives the click) *before* the feed framework finishes hydrating that node's own click handler, so the dispatched click is silently swallowed — `.click()` reports success, but no composer ever opens (issue #150; seen as `_upload_photo`'s `input[type="file"]` wait timing out with the feed screenshot still showing the bare share box, "Photo" visibly focused). `click_feed_entry`'s optional `expect_selector` kwarg closes this gap: after a successful click it also waits (short timeout) for `expect_selector` to attach, and treats a no-show as an inert click — retrying within the same overall budget instead of returning a false success. All three share-box callers pass one (issue #271): `_click_add_photo` and `_click_video_button` wait for `MEDIA_FILE_INPUT_SEL` (Video mounts its file input straight away since 2026-09, like Photo), and `_click_start_a_post` waits for `COMPOSER_EDITOR_SEL`, the caption editor scoped to an open modal. Both selectors live in `linkedin_labels.py`. Since 2026-09 every feed load logs React error #418 (hydration mismatch), so swallowed clicks are no longer rare. Before re-clicking, `click_feed_entry` checks the effect once more, and an effect that merely landed late is accepted: a re-click would hit the editor's modal, which now covers the share box, and burn the whole budget on intercepted clicks.
...
4. In the Schedule dialog, click the **Scheduled (N)** tab at the top (this replaced the old "View all scheduled posts" link in the 2026 rebuild).
