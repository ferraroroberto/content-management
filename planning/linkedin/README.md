...
| step | selector |
|------|----------|
| Open empty composer (no Photo) | `click_feed_entry(page, START_POST_TEXT_RE, "Start a post")` → `page.get_by_text(START_POST_TEXT_RE)` (EN `start a post` / `create a post` / ES `empieza una publicación` / `crea una publicación`) — also role-less post-hydration (issue #140) |
...
- **A feed-entry click can succeed and still do nothing.** Distinct from #27/#140: the target `<a onclick>` can pass every Playwright actionability check (attached, visible, stable, receives the click) *before* the feed framework finishes hydrating that node's own click handler, so the dispatched click is silently swallowed — `.click()` reports success, but no composer ever opens (issue #150; seen as `_upload_photo`'s `input[type="file"]` wait timing out with the feed screenshot still showing the bare share box, "Photo" visibly focused). `click_feed_entry`'s optional `expect_selector` kwarg closes this gap: after a successful click it also waits (short timeout) for `expect_selector` to attach, and treats a no-show as an inert click — retrying within the same overall budget instead of returning a false success. Only `_click_add_photo` passes it today (`input[type="file"]`); extend `_click_start_a_post` / `_click_video_button` the same way if they start showing the same symptom.
