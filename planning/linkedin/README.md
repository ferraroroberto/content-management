...
| Wait until Done is clickable | `DONE_BTN_RE` stays `disabled` until the PDF is processed **and** a title is present, so this runs *after* the title is filled — polling it first deadlocks |
| Close document dialog (Done) | `_dialog(page) >> get_by_role("button", name=DONE_BTN_RE)` |
