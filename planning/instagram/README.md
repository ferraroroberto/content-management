...
| Open Schedule menu | The day-column `Schedule ▾` is a split-button (`aria-haspopup="menu"`). Its menu — `Schedule post` / `Schedule story` / `Create reel` / `Schedule ad` — only opens on a **JS-native** `el.click()`; Playwright's synthetic mouse `.click()` leaves it closed (same half-bound React handler as the Add-media button, #28). Opening via Playwright click is why story scheduling silently timed out while post appeared to work. |
| Pick menu item | After the JS-native open, `page.get_by_role("menuitem", name=/^Schedule story$/i)` (fallback: `get_by_text("Schedule story")`), retried by re-opening the menu with escalating 8s/16s/32s click timeouts. |
