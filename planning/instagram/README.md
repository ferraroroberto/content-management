...
| Pick menu item | `page.get_by_role("menuitem", name=/^Schedule story$/i)` (fallback: `get_by_text("Schedule story")`), retried with escalating 8s/16s/32s timeouts — Meta's menu intermittently renders slowly or collapses, so a single 8s attempt flakes; each retry re-pops the menu via the Schedule button if it has collapsed. |
