...
from notion.editorial import (  # noqa: E402
    get_field,
    init_notion_client,
...
from notion.notion_update import format_database_id  # noqa: E402
...
def fetch_wip_ig_rows(notion, db_id: str, ed_cols: dict, days: list[date]) -> list[ScheduleRow]:
...
def schedule_story(
    ...
    *,
    ...
    out_dir = Path(__file__).resolve().parent.parent / "results" / "instagram"
...
def main() -> int:
    ...
    with InstagramSession(cfg) as session:
        ...
        for row, story, post in plans:
            ...
            if not dry_run and all("LIVE" in s for s in day_statuses):
                try:
                    set_field(
    ...
    for s in statuses:
        logger.info("   %s", s)
    failed = [s for s in statuses if "FAIL" in s]
    return 0 if not failed else 11
