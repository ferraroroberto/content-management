...
flowchart TD
    ...
    Clone[<b>run:</b><br/>python -m instagram.clone_to_other_platforms<br/>--week-start YYYY-MM-DD --live]
    ...
    Run[<b>run:</b><br/>python -m instagram.schedule_instagram_posts<br/>--week-start YYYY-MM-DD --live]
...
```powershell
& .\.venv\Scripts\python.exe -m instagram.bootstrap_session
```
...
& .\.venv\Scripts\python.exe -m instagram.clone_to_other_platforms --week-start 2026-05-25 --dry-run
& .\.venv\Scripts\python.exe -m instagram.clone_to_other_platforms --week-start 2026-05-25 --live

# 2) Drive the Meta planner for IG (story + post per day)
& .\.venv\Scripts\python.exe -m instagram.schedule_instagram_posts                        # default = dry-run, next Monday
& .\.venv\Scripts\python.exe -m instagram.schedule_instagram_posts --week-start 2026-05-25 --dry-run
& .\.venv\Scripts\python.exe -m instagram.schedule_instagram_posts --week-start 2026-05-25 --live
& .\.venv\Scripts\python.exe -m instagram.schedule_instagram_posts --date 20260518 --live  # single-day testing
& .\.venv\Scripts\python.exe -m instagram.schedule_instagram_posts --date 20260518 --live --force  # ignore link IG
```
...
| Add photo/video | `page.expect_file_chooser()` wrapping a click on `div[role="button"]:has-text("Add photo/video")`. Meta does NOT pre-mount an `input[type=file]`; the click opens the OS file dialog and Playwright intercepts it as a FileChooser. |
...
- **Meta opens its file picker on click — there's no addressable `input[type=file]` in advance.** Use `page.expect_file_chooser()` around the `Add photo/video` click and feed the files via `file_chooser.set_files([...])`. `dispatch_event('click')` doesn't trigger Meta's React handler that mounts the input.
