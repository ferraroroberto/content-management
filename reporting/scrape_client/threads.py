...
def _post_engagement(post_text: str) -> dict:
    """Extract likes / comments / reshares from a thread's inner text.

    Threads renders the engagement bar as four icon-buttons; each visible
    count sits adjacent to the icon. We parse the raw text in the post
    container, which on a typical own-thread contains lines of the form
    ``"13"``, ``"2"``, ``"3"`` after the body text.

    The order on threads.com is: like → reply → repost → share. We rely on
    role/aria-label selectors first, then fall back to crude positional parsing.
    """
    return {"num_likes": None, "num_comments": None, "num_reshares": None}


def _extract_engagement_from_buttons(post_locator) -> dict:
    """Pull counts from the Threads engagement bar.

    Threads buttons carry no ``aria-label``. Each engagement slot is a
    ``<div>`` that wraps an SVG icon and a count span. The four slots are in
    fixed order: like, reply, repost, share. We extract the bottom-most run of
    button-like divs (those containing an ``<svg>``) from the post container
    and read each one's visible text. Empty text → 0.
    """
    out = {"num_likes": 0, "num_comments": 0, "num_reshares": 0}
    try:
        # Use a JS extractor so we can walk the DOM precisely. Returns a list
        # of [text, ...] in document order for the engagement slots.
        texts = post_locator.evaluate(
            """el => {
                // Find LEAF slot-divs: each engagement slot is a div that
                // (a) directly contains an SVG icon,
                // (b) does NOT contain any descendant div that has an SVG
                //     (so we exclude wrapper/parent containers), and
                // (c) has text that's either empty (count=0) or pure digits.
                const allDivs = Array.from(el.querySelectorAll('div'));
                const slots = allDivs.filter(d => {
                    if (!d.querySelector('svg')) return false;
                    const innerSvgDivs = Array.from(d.querySelectorAll('div'))
                        .some(d2 => d2.querySelector('svg'));
                    if (innerSvgDivs) return false;  // not a leaf slot
                    const txt = (d.innerText || '').trim();
                    return /^[\\d][\\d,. ]*$|^$/.test(txt);
                });
                if (slots.length < 3) return [];
                // The engagement bar is the LAST horizontal group of >=4
                // (or >=3 if the share slot was filtered) leaf slots.
                const last4 = slots.slice(-4);
                return last4.map(d => (d.innerText || '').trim());
            }"""
        )
        if not isinstance(texts, list) or len(texts) < 3:
            return out
        # Map slot positions to canonical fields.
        if len(texts) >= 1:
            v = parse_int(texts[0])
            out["num_likes"] = v if v is not None else 0
        if len(texts) >= 2:
            v = parse_int(texts[1])
            out["num_comments"] = v if v is not None else 0
        if len(texts) >= 3:
            v = parse_int(texts[2])
            out["num_reshares"] = v if v is not None else 0
    except Exception as err:
        logger.debug("Threads engagement JS-extract failed: %s", err)
    return out


def _post_permalink_and_date(post_locator, handle: str) -> tuple[Optional[str], Optional[str]]:
    """Find the post's permalink ``/@handle/post/<code>`` and the ``<time datetime>``."""
    post_url = None
    posted_at = None
    try:
        anchors = post_locator.locator("a[href*='/post/']")
    ...
    for i in range(n):
        ...
        if m and m.group(1).lower() == handle.lower():
            ...
            break
    try:
        time_el = post_locator.locator("time").first
        if time_el.count() > 0:
